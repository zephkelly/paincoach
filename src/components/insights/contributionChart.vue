<template>
    <section class="contribution-chart panel">
        <div class="headers wrapper">
            <h2>Largest pain contributors</h2>
        </div>
            ...
            <div class="label wrapper">
                <div v-for="(item, index) in dataMinusUnknown" :key="index" :class="`label label-${index}`" :style="getLabelStyle(item)">
                    <div class="label-content" :style="`background-color: ${item.color}`">
                        ...
                        <p class="label-percentage">{{ item.value }}%</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="factors wrapper">
            <ul>    
                <li v-for="(item, index) in dataMinusUnknown" :key="index" :class="`factor-secondary-label factor-${index}`">
                    <h3>
                        <span :class="item.id">{{ item.label }}</span><template v-if="index !== data.length - 1">,</template><template v-else>.</template>
                    </h3>
                </li>
            </ul>
        </div>
...
<script setup>
    ...
    const data = [
        { id: 'psychological', label: 'Psychological Distress', value: 50, color: '#D24646' },
        { id: 'exercise', label: 'Exercise', value: 16, color: '#ef8e56' },
        { id: 'sleep', label: 'Sleep', value: 14, color: '#E8CB9F' },
        { id: 'unknown', label: 'Other', value: 20, color: '#58585829' },
    ]
